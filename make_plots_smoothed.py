#!/usr/bin/env python3
"""
V1 vs V2 (smoothed) predicate timeline heatmap.
Left  = V1 (annotation-based, per-object binary).
Right = V2 smoothed (median filter + monotonic hold).
...
V2_DIR = "/vast/home/y/yishao/Vincent/v2_predicate_verify/predicates_smoothed"
OUT_DIR = "/vast/home/y/yishao/Vincent/v2_predicate_verify/comparison_smoothed"
...
def load_v2(task_id, episode_id):
    ...
    with open(path) as f:
        for line in f:
            ...
            rows.append([pred_value(p) for p in r["predicates"]])
...
def plot_one(task_id, episode_id, out_path):
    ...
    v1_states, v1_labels = load_v1(task_id, episode_id)
    v2_values, v2_labels, v2_steps = load_v2(task_id, episode_id)
    ...
    ax_v2.set_title(
        f"V2 smoothed: {N2} per-predicate continuous  ({T2} steps)",
        fontsize=11, color="#0b3d91", fontweight="bold",
    )
    ...
    fig.suptitle(
        f"Episode {episode_id} \u2014 {task_name}: V1 vs V2 Predicate Timeline (smoothed)",
        fontsize=14, color="#0b3d91", fontweight="bold", y=0.965,
    )
    fig.text(
        0.5, 0.018,
        "V1 = per-object binary (annotation-based) | V2 = per-predicate "
        "continuous progress (smoothed: median k=5 + monotonic hold=10)",
...
def common_episodes(task_id, n=1):
    with open(f"{V1_DIR}/task_{task_id:04d}_state_action_vectors.pkl", "rb") as f:
        v1 = pickle.load(f)
    ...
    for tid in V2_TASKS:
        ...
        for ep in eps:
            ...
            try:
                ...
                print(f"  ERR {os.path.basename(out)}: {e}")
    print(f"\n{n_done} plots generated.")
